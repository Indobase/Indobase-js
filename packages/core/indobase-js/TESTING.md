...
npx nx test:integration indobase-js
npx nx test:deno:all indobase-js
npx nx test:edge-functions indobase-js

# 3. Cleanup when done
npx nx test:indobase:stop indobase-js
```

### What `test:indobase:setup` Does
...
5. ✅ Exports `INDOBASE_ANON_KEY` and `INDOBASE_SERVICE_ROLE_KEY` environment variables
...
| -------------------------- | ----------------------------------------- | -------------------------------------- |
| `test:indobase:setup`      | **Setup Indobase once** for all tests     | Docker running, Indobase CLI installed |
| `test:indobase:stop`       | **Cleanup** Indobase after testing        | None                                   |
...
2. **Indobase CLI** must be installed (`brew install indobase/tap/indobase` - [for other platforms read here](https://indobase.com/docs/guides/local-development/cli/getting-started?queryGroups=platform&platform=macos#installing-the-indobase-cli))
...
npx nx test:unit indobase-js

# Run tests in watch mode during development
npx nx test indobase-js --watch

# Run tests with coverage report
npx nx test:coverage indobase-js
```
...
# 3. Run browser tests
npx nx test:integration:browser indobase-js
...
```bash
# Navigate to the indobase-js package directory
cd packages/core/indobase-js

# Start Indobase
npx indobase start

# Serve edge functions in background (if needed)
npx indobase functions serve &

# Export keys for tests
export INDOBASE_ANON_KEY="$(npx indobase status --output json | jq -r '.ANON_KEY')"
export INDOBASE_SERVICE_ROLE_KEY="$(npx indobase status --output json | jq -r '.SERVICE_ROLE_KEY')"
...
```bash
# 1. Setup Indobase once
npx nx test:indobase:setup indobase-js

# 2. Run all local tests (unit + integration + browser)
npx nx test:all indobase-js
...
- [x] Docker running
- [x] Indobase CLI installed
- [x] jq installed (for JSON parsing)
- [x] Run `test:indobase:setup` first (handles keys automatically)

**Common Issues and Solutions:**

| Issue                                        | Solution                                                                      |
| -------------------------------------------- | ----------------------------------------------------------------------------- |
| "Port 54322 already allocated"               | Run `npx nx test:indobase:stop indobase-js` or `npx indobase stop`            |
| "503 Service Unavailable" for Edge Functions | Run `npx nx test:indobase:setup indobase-js` (serves functions automatically) |
| Integration tests fail with auth errors      | Run `npx nx test:indobase:setup indobase-js` (exports keys automatically)     |
| "jq: command not found"                      | Install jq: `brew install jq` (macOS) or `apt-get install jq` (Linux)         |
| Functions log filling up disk                | Logs are in `/tmp/indobase-functions.log`, cleanup removes them               |
...
import { corsHeaders } from '@indobase/indobase-js/cors'
```

The `indobase/deno.json` file maps this import to pkg.pr.new URLs:

```json
{
  "imports": {
    "@indobase/indobase-js/cors": "https://esm.sh/pr/indobase/indobase-js/@indobase/indobase-js@{COMMIT_HASH}/dist/cors.mjs"
  }
}
```
...
git checkout packages/core/indobase-js/indobase/deno.json
git checkout packages/core/indobase-js/test/deno/deno.json
```
...
- ✅ `test:indobase:setup` automatically serves Edge Functions in the background
- ✅ Functions are served on `http://127.0.0.1:54321/functions/v1/<function-name>`
- ✅ Function logs are written to `/tmp/indobase-functions.log`
- ❌ Edge Functions tests will fail with 503 errors if setup hasn't been run
- ❌ Running `npx indobase start` alone does NOT serve edge functions
...
```bash
# View function logs
tail -f /tmp/indobase-functions.log

# Test a function directly
curl -X POST \
  -H "Authorization: Bearer $(npx indobase status --output json | jq -r '.ANON_KEY')" \
...
{
  "imports": {
    "@indobase/indobase-js": "https://esm.sh/pr/indobase/indobase-js/@indobase/indobase-js@{COMMIT_HASH}?target=deno"
  }
}
```
...
npx nx serve:coverage indobase-js
