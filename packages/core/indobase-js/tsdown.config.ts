...
export default defineConfig([
  // CJS and ESM builds - keep @indobase/* external
  {
    ...
    clean: true,
    outDir: 'dist',
    // CRITICAL: Keep @indobase/* packages external - not bundled
    external: [
      '@indobase/auth-js',
  ...
  {
    entry: { indobase: 'src/index.ts' },
    format: ['iife'],
    globalName: 'indobase',
    outDir: 'dist/umd',
    // Bundle all dependencies including @indobase/*
    noExternal: [/.*/],
    minify: true,
    platform: 'browser',
    // Rename to indobase.js for backward compatibility (can remove in v3)
    onSuccess: async () => {
      const { rename, access } = await import('fs/promises')
      const { constants } = await import('fs')
      const sourceFile = join(__dirname, 'dist/umd/indobase.iife.js')
      const destFile = join(__dirname, 'dist/umd/indobase.js')
