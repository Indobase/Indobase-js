...
echo "🚀 Stop indobase if it is running"
npx indobase stop

echo "🚀 Starting Indobase (migrations applied, seed data skipped)..."
npx indobase start
...
npx indobase functions serve --import-map indobase/deno.json > /tmp/indobase-functions.log 2>&1 &
FUNCTIONS_PID=$!
echo $FUNCTIONS_PID > /tmp/indobase-functions.pid
...
for i in {1..3}; do
  RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
    -H "Authorization: Bearer $INDOBASE_ANON_KEY" \
