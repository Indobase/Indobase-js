...
[![Package](https://img.shields.io/npm/v/@indobase/indobase-js)](https://www.npmjs.com/package/@indobase/indobase-js)
...
```html
<script src="https://cdn.jsdelivr.net/npm/@indobase/indobase-js@2"></script>
```

or even:

```html
<script src="https://unpkg.com/@indobase/indobase-js@2"></script>
```
...
```html
<script type="module">
  import { createClient } from 'https://cdn.jsdelivr.net/npm/@indobase/indobase-js/+esm'
  const client = createClient('https://xyzcompany.indobase.fun', 'public-anon-key')
...
You can use the SDK in the Deno runtime via [JSR](https://jsr.io/@indobase/indobase-js):

```js
import { createClient } from 'jsr:@indobase/indobase-js@2'
```
...
`@indobase/indobase-js` re-exports `PostgrestError`, `FunctionsError`, and related symbols so you can import them directly from `@indobase/indobase-js`. However, our build tool merges all imports from the same package into a single import statement in the built output:
