...
<body>
    ...
    <script src="http://localhost:${port}/indobase.js"></script>
    ...
    <script type="text/babel" data-presets="env,react">
        ...
        const indobase = window.indobase.createClient(INDOBASE_URL, ANON_KEY, {
            realtime: {
        ...
        const App = () => {
            ...
            React.useEffect(() => {
                ...
                const channel = indobase.channel(channelName, {
...
beforeAll(async () => {
  await new Deno.Command('indobase', { args: ['start'], stderr }).output()
  ...
  serve(
    async (req: any) => {
      if (req.url.endsWith('indobase.js')) {
        const file = await Deno.readFile('./dist/umd/indobase.js')
