...
describe('Indobase Integration Tests', () => {
  ...
  describe('PostgREST', () => {
    ...
    test('should create and delete a todo', async () => {
      ...
      const { error: deleteError } = await indobase.from('todos').delete().eq('id', createdTodo!.id)
  ...
  describe('PostgreSQL RLS', () => {
    ...
    beforeAll(async () => {
      ...
      const { data: user1Data } = await indobase.auth.signUp({
        email: user1Email,
        password,
      })
      user1Id = user1Data.user!.id

      const { data: user2Data } = await indobase.auth.signUp({
        email: user2Email,
        password,
      })
      ...
      await indobase.auth.signInWithPassword({ email: user1Email, password })
      const { data: user1Todo } = await indobase
        .from('todos')
      ...
      await indobase.auth.signInWithPassword({ email: user2Email, password })
      const { data: user2Todo } = await indobase
        .from('todos')
    ...
    test('should allow authenticated user to access their own data', async () => {
      await indobase.auth.signInWithPassword({ email: user1Email, password: 'password123' })
  ...
  describe.each([{ vsn: '1.0.0' }, { vsn: '2.0.0' }])('Realtime with vsn: $vsn', ({ vsn }) => {
    ...
    beforeEach(async () => {
      ...
      indobase = createClient(INDOBASE_URL, ANON_KEY, {
        realtime: {
          heartbeatIntervalMs: 500,
          vsn,
          ...(wsTransport && { transport: wsTransport }),
      ...
      channel = indobase.channel(channelName, { config })
    })
    ...
    test('is able to connect and broadcast', async () => {
      ...
      expect(indobase.realtime.getChannels().length).toBe(1)
    }, 10000)
  })
})
...
describe('Storage API', () => {
  ...
  const SERVICE_ROLE_KEY = process.env.INDOBASE_SERVICE_ROLE_KEY || 'use-service-role-key'
...
describe('PostgREST Timeout Configuration', () => {
  test('should accept timeout option through client configuration', () => {
    const client = createClient(INDOBASE_URL, ANON_KEY, {
      db: { timeout: 5000 },
    })
...
describe('Custom JWT', () => {
  describe('Realtime', () => {
    test('will connect with a properly signed jwt token', async () => {
      ...
      const indobaseWithCustomJwt = createClient(INDOBASE_URL, ANON_KEY, {
      ...
      try {
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          indobaseWithCustomJwt.channel('test-channel').subscribe((status, err) => {
            if (status === 'SUBSCRIBED') {
              clearTimeout(timeout)
              // Verify token was set
              expect(indobaseWithCustomJwt.realtime.accessTokenValue).toBe(jwtToken)
              resolve()
        ...
        await indobaseWithCustomJwt.removeAllChannels()
      }
    }, 5000)
  })
})
