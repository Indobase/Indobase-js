...
export async function updateSession(request: NextRequest) {
  ...
  const indobase = createServerClient(
    ...
    process.env.NEXT_PUBLIC_INDOBASE_ANON_KEY!,
    {
      cookies: {
        getAll() {
  ...
  // indobase.auth.getUser(). A simple mistake could make it very hard to debug
  ...
  // IMPORTANT: You *must* return the indobaseResponse object as it is.
  // If you're creating a new response object with NextResponse.next() make sure to:
  // 1. Pass the request in it, like so:
  //    const myNewResponse = NextResponse.next({ request })
  // 2. Copy over the cookies, like so:
  //    myNewResponse.cookies.setAll(indobaseResponse.cookies.getAll())
