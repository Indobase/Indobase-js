...
describe('constants', () => {
  ...
  test('DEFAULT_HEADERS should contain X-Client-Info', () => {
    ...
    expect(DEFAULT_HEADERS['X-Client-Info']).toMatch(/^indobase-js-.*\/.*$/)
  })
  ...
  describe('JS_ENV detection', () => {
    test('should detect Deno environment', () => {
      ...
      expect(newHeaders['X-Client-Info']).toContain('indobase-js-deno')
    })
    ...
    test('should detect web environment', () => {
      ...
      expect(newHeaders['X-Client-Info']).toContain('indobase-js-web')
    })
    ...
    test('should detect React Native environment', () => {
      ...
      expect(newHeaders['X-Client-Info']).toContain('indobase-js-react-native')
    })
    ...
    test('should default to node environment when no specific environment is detected', () => {
      ...
      expect(newHeaders['X-Client-Info']).toContain('indobase-js-node')
    })
  })
})
