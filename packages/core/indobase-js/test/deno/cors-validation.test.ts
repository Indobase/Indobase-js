...
// 1. Run `indobase start` from packages/core/indobase-js
// 2. The cors-validation function will be automatically deployed
// 3. Run `nx test:deno:cors-validation indobase-js` from the repository root
...
Deno.test(
  ...
  async (t) => {
    ...
    await t.step('OPTIONS preflight - should include all Indobase SDK headers', async () => {
      const response = await fetch(functionUrl, {
        method: 'OPTIONS',
      })
      ...
      // All 10 current Indobase headers must be allowed
    ...
    await t.step('POST request - should accept all Indobase SDK headers', async () => {
      const response = await fetch(functionUrl, {
        method: 'POST',
        headers: {
          'Authorization': 'Bearer test-token',
          'x-client-info': '@indobase/indobase-js/2.0.0',
          'apikey': 'test-api-key',
    ...
    await t.step('POST request - should prevent v2.93.0 incident (NEW headers accepted)', async () => {
      // This test simulates adding a new header to the SDK (like x-indobase-client-platform)
      ...
      const preflightResponse = await fetch(functionUrl, {
        ...
        headers: {
          'Origin': 'https://example.com',
          'Access-Control-Request-Method': 'POST',
          'Access-Control-Request-Headers': 'authorization, x-indobase-new-feature',
        },
      })
      ...
      if (allowedHeaders !== '*') {
        ...
        if (!allowedSet.has('x-indobase-new-feature')) {
          throw new Error(
            `New header "x-indobase-new-feature" not found in Access-Control-Allow-Headers: ${allowedHeaders}`
          )
        }
      }
      ...
      const response = await fetch(functionUrl, {
        ...
        headers: {
          'Authorization': 'Bearer test-token',
          'x-client-info': '@indobase/indobase-js/2.0.0',
          'x-indobase-new-feature': 'test-value', // NEW HEADER (example)
          'Content-Type': 'application/json',
        },
      ...
      assertEquals(data.receivedHeaders['x-indobase-new-feature'], 'test-value')
    })
